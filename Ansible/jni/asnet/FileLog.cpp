/*
 * This is the source code of asnet library v. 1.1
...
#ifdef ANDROID
    __android_log_vprint(ANDROID_LOG_FATAL, "asnet", message, argptr);
    va_end(argptr);
    va_start(argptr, message);
#else
...
void FileLog::e(const char *message, ...) {
    ...
    struct tm *now = localtime(&time_now.tv_sec);
#ifdef ANDROID
    __android_log_vprint(ANDROID_LOG_ERROR, "asnet", message, argptr);
    va_end(argptr);
    va_start(argptr, message);
#else
...
void FileLog::w(const char *message, ...) {
    ...
    struct tm *now = localtime(&time_now.tv_sec);
#ifdef ANDROID
    __android_log_vprint(ANDROID_LOG_WARN, "asnet", message, argptr);
    va_end(argptr);
    va_start(argptr, message);
#else
...
void FileLog::d(const char *message, ...) {
    ...
    struct tm *now = localtime(&time_now.tv_sec);
#ifdef ANDROID
    __android_log_vprint(ANDROID_LOG_DEBUG, "asnet", message, argptr);
    va_end(argptr);
    va_start(argptr, message);
#else
...
void FileLog::ref(const char *message, ...) {
    ...
    __android_log_vprint(ANDROID_LOG_VERBOSE, "asnetREF", s.str().c_str(), argptr);
    va_end(argptr);
    va_start(argptr, message);
#endif
    va_end(argptr);
