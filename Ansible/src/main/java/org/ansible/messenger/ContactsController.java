...
public class ContactsController extends BaseController {
    ...
    public String getInviteText(int contacts) {
        String link = inviteLink == null ? "https://ansible.su/dl" : inviteLink;
        if (contacts <= 1) {
    ...
    public void checkAppAccount() {
        ...
        Utilities.globalQueue.postRunnable(() -> {
            ...
            try {
                Account[] accounts = am.getAccountsByType("org.ansible.messenger");
            ...
            if (getUserConfig().isClientActivated()) {
                ...
                if (systemAccount == null) {
                    try {
                        systemAccount = new Account("" + getUserConfig().getClientUserId(), "org.ansible.messenger");
    ...
    protected void performSyncPhoneBook(final HashMap<String, Contact> contactHashMap, final boolean request, final boolean first, final boolean schedule, final boolean force, final boolean checkCount, final boolean canceled) {
        ...
        Utilities.globalQueue.postRunnable(() -> {
            ...
            /*if (schedule) {
                try {
                    AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
                    Account[] accounts = am.getAccountsByType("org.ansible.account");
                    boolean recreateAccount = false;
                    if (getUserConfig().isClientActivated()) {
                        if (accounts.length != 1) {
                            FileLog.e("detected account deletion!");
                            currentAccount = new Account(getUserConfig().getCurrentUser().phone, "org.ansible.account");
...
//                });
    ...
    private void applyContactToPhoneBook(ArrayList<ContentProviderOperation> query, TLRPC.User user) {
        ...
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.org.ansible.messenger.android.profile");
        builder.withValue(ContactsContract.Data.DATA1, user.id);
        builder.withValue(ContactsContract.Data.DATA2, "Ansible Profile");
        ...
        builder = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI);
        builder.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, rawContactId);
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.org.ansible.messenger.android.call");
        builder.withValue(ContactsContract.Data.DATA1, user.id);
        builder.withValue(ContactsContract.Data.DATA2, "Ansible Voice Call");
        ...
        builder = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI);
        builder.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, rawContactId);
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.org.ansible.messenger.android.call.video");
        builder.withValue(ContactsContract.Data.DATA1, user.id);
        builder.withValue(ContactsContract.Data.DATA2, "Ansible Video Call");
    ...
    public void createOrUpdateConnectionServiceContact(long id, String firstName, String lastName) {
        ...
        try {
            ...
            Cursor cursor = resolver.query(groupsURI, new String[]{ContactsContract.Groups._ID},
                    ...
                    new String[]{"AnsibleConnectionService", systemAccount.type, systemAccount.name}, null);
            int groupID;
            if (cursor != null && cursor.moveToFirst()) {
                ...
                values.put(ContactsContract.Groups.TITLE, "AnsibleConnectionService");
