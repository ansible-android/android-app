...
object SchemeCodeParser {
    fun parse(files: List<File>): SchemeTlClasses {
        return SchemeTlClasses(files.map { findAllClasses(it) }.flatten().toSet())
    }

    private fun findAllClasses(javaFile: File): List<SchemeTlClass> {
        ...
        val matchingClassNames = mutableListOf<SchemeTlClass>()
        ...
        for (cls in allClassLike) {
            ...
            if (hasStaticIntConstructor || hasDeserializeMethod || hasSerializeToStream || hasReadParams || hasDeserializeResponse) {
                matchingClassNames.add(SchemeTlClass(
